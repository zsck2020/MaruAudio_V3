      ...
      <el-col :span="24" style="margin-bottom: 20px;">
        <div class="table-card">
          <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 15px;">
            <h4 style="margin: 0;">
              <el-icon><Picture /></el-icon> Banner 轮播管理
            </h4>
            <el-button type="primary" size="small" @click="showBannerDialog()">+ 添加 Banner</el-button>
          </div>
          <el-alert type="info" :closable="false" style="margin-bottom: 15px;">
            <template #title>管理客户端首页轮播图，支持拖拽排序、启用/禁用、配置跳转链接</template>
          </el-alert>
          
          <el-table :data="bannerList" style="width: 100%" v-loading="bannerLoading" empty-text="暂无Banner">
            <el-table-column label="排序" width="70" align="center">
              <template #default="{ row }">
                <span style="color: #999; cursor: move;">{{ row.sort_order }}</span>
              </template>
            </el-table-column>
...
import { Plus, Connection, Upload, Picture } from '@element-plus/icons-vue'
import { getSettings, updateSettings, getBanners, createBanner, updateBanner, deleteBanner, toggleBanner } from '../api'
